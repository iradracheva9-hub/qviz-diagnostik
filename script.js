...
function loadLeadSettings() {
  const fallback = {
    title: "Почти готово!",
    subtitle:
      "Заполни форму ниже чтобы получить рекомендации и понять где ты правда устаёшь не от жизни, а от чужих ожиданий, чужого темпа, чужих эмоций и роли “хорошей девочки”.",
    nameLabel: "Имя",
    namePlaceholder: "Как к вам обращаться",
    telegramLabel: "Никнейм Telegram",
  ...
  try {
    ...
    return {
      ...
      subtitle:
        typeof parsed.subtitle === "string" && parsed.subtitle.trim() ? parsed.subtitle.trim() : fallback.subtitle,
      nameLabel:
        typeof parsed.nameLabel === "string" && parsed.nameLabel.trim() ? parsed.nameLabel.trim() : fallback.nameLabel,
      namePlaceholder:
        typeof parsed.namePlaceholder === "string" && parsed.namePlaceholder.trim()
          ? parsed.namePlaceholder.trim()
          : fallback.namePlaceholder,
      telegramLabel:
        typeof parsed.telegramLabel === "string" && parsed.telegramLabel.trim()
          ? parsed.telegramLabel.trim()
          : fallback.telegramLabel,
      telegramPlaceholder:
        typeof parsed.telegramPlaceholder === "string" && parsed.telegramPlaceholder.trim()
          ? parsed.telegramPlaceholder.trim()
          : fallback.telegramPlaceholder,
      emailLabel:
        typeof parsed.emailLabel === "string" && parsed.emailLabel.trim()
          ? parsed.emailLabel.trim()
          : fallback.emailLabel,
      emailPlaceholder:
        typeof parsed.emailPlaceholder === "string" && parsed.emailPlaceholder.trim()
          ? parsed.emailPlaceholder.trim()
...
function initQuizPage() {
  ...
  const leadTelegramLabel = document.getElementById("lead-telegram-label");
  ...
  leadNameLabel.textContent = lead.nameLabel;
  leadTelegramLabel.textContent = lead.telegramLabel;
  leadEmailLabel.textContent = lead.emailLabel;
  nameInput.placeholder = lead.namePlaceholder;
  telegramInput.placeholder = lead.telegramPlaceholder;
  emailInput.placeholder = lead.emailPlaceholder;
  ...
  window.addEventListener("storage", (event) => {
    ...
    if (event.key === STORAGE_KEYS.lead) {
      ...
      leadNameLabel.textContent = nextLead.nameLabel;
      leadTelegramLabel.textContent = nextLead.telegramLabel;
      leadEmailLabel.textContent = nextLead.emailLabel;
      nameInput.placeholder = nextLead.namePlaceholder;
      telegramInput.placeholder = nextLead.telegramPlaceholder;
      emailInput.placeholder = nextLead.emailPlaceholder;
...
function initAdminPage() {
  ...
  const leadNameLabelInput = document.getElementById("lead-name-label-input");
  const leadNamePlaceholderInput = document.getElementById("lead-name-placeholder-input");
  const leadTelegramLabelInput = document.getElementById("lead-telegram-label-input");
  const leadTelegramPlaceholderInput = document.getElementById("lead-telegram-placeholder-input");
  const leadEmailLabelInput = document.getElementById("lead-email-label-input");
  const leadEmailPlaceholderInput = document.getElementById("lead-email-placeholder-input");
  ...
  function openPanel() {
    ...
    leadNameLabelInput.value = lead.nameLabel;
    leadNamePlaceholderInput.value = lead.namePlaceholder;
    leadTelegramLabelInput.value = lead.telegramLabel;
    leadTelegramPlaceholderInput.value = lead.telegramPlaceholder;
    leadEmailLabelInput.value = lead.emailLabel;
    leadEmailPlaceholderInput.value = lead.emailPlaceholder;
  ...
  adminSaveBtn.addEventListener("click", () => {
    ...
    const leadSettings = {
      ...
      nameLabel: leadNameLabelInput.value.trim(),
      namePlaceholder: leadNamePlaceholderInput.value.trim(),
      telegramLabel: leadTelegramLabelInput.value.trim(),
      telegramPlaceholder: leadTelegramPlaceholderInput.value.trim(),
      emailLabel: leadEmailLabelInput.value.trim(),
      emailPlaceholder: leadEmailPlaceholderInput.value.trim(),
